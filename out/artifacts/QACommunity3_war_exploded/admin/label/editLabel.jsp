...
<div class="row">
    ...
    <div class="col-md-12">
        <div class="card">
            <form id="editLabelForm" class="form-horizontal" method="post" novalidate="" onsubmit="return false">
            <s:iterator value="qaTheLabel">
                            ...
                            <div class="col-sm-6">
                                <select name="topicId" class="selectpicker" data-title="选择话题" required="required" data-style="btn-default btn-block" data-menu-style="dropdown-blue">
                                    <s:iterator value="qaTopicList" var="topic">
                                        <option value="<s:property value="#topic.toId"/>" <s:if test="topicId == #topic.toId">selected</s:if> ><s:property value="#topic.topicName"/></option>
                                    </s:iterator>
                                </select>
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-6">
                                <input class="form-control"
                                       ...
                                       name="labelName"
                                       required="required"
                                       value="<s:property value="labelName"/>"
                                />
                            </div>
                ...
                <div class="footer text-center">
                    <button type="submit" class="btn btn-info btn-fill editHandle">提交</button>
                </div>
...
<script>
    $().ready(function(){
        <%--$.ajax({--%>
        <%--type:"POST",--%>
        <%--url:"<%=path%>/admin/qaBackLabel_topicSelect.action",--%>
        <%--dataType: "json",--%>
        <%--data: "",--%>
        ...
        $('.editHandle').on('click', function() {
            if($("#editLabelForm").valid()) {  //表单验证通过
                $.ajax({
                    type:"POST",
                    url:"<%=path%>/admin/qaBackLabel_updateLabel.action",
                    dataType: "json",
                    data: $("form").serialize(),
                    success: function(data) {
                        var data2 = eval("("+data+")");  //转化为json对象
